...
class OfficerController  extends BaseController
{
    ...
    public function profile($status, $state, $level, $page = 0)
    {
        if (Auth::user()->mainRole()->name === 'officer') {
            ...
            return view('officer::pages.applicant-profile-list', compact(
                'datas',
                'state',
                'status',
                'countTSLC',
    ...
    public function edit($id)
    {
        if (Auth::user()->mainRole()->name === 'officer') {
            ...
            $examState = $this->examProcessingRepository->getAll()->where('profile_id', '=', $id)->first();;
    ...
    public function examDetails($id)
    {
        ...
        $phy = ExamProcessing::join('profiles', 'profiles.id', '=', 'exam_registration.profile_id')
            ...
            ->count(['profiles.id']);
        return view('officer::pages.exam.show', compact(
            'appliedCount',
            'rejectedCount',
            'failedCount',
            'operatorState',
