            ...
            <div class="id-card-conten" style="width:400px; height : 250px; padding:10px; background:white; border:1px solid black;" id="printContent"> 
              <div class= "header" style="text-align: center; width:100%;">
                ...
                <div class="logo" style="width: 15%;float:left; justify-content:center; margin-top:10px;">
                    {!! \SimpleSoftwareIO\QrCode\Facades\QrCode::size(60)->generate('Name :- ' .  $data->name . ' Date of Birth :- ' .$profile->dob_nep . ' Citizenship Number : - ' . $profile->citizenship_number . ' Decision Date ' . $data->decision_date . ' Licence Number : -' . $data->cert_registration_number) !!}
                </div>
              </div>
              <div class= "body" style="width:100%; margin-top:20px;">
                <div class="logo" style="line-height: 1.8; width: 80%; float:left; font-size:11px;">
                    ...
                    <span>Name       : <span style="font-weight:700;"> {{ $data->name }} </span></span></br>
                    <span>Level      : <span style="font-weight:700;"> {{ $data->level_name }} </span></span></br>
                    <span>Discipline (Sub): <span style="font-weight:700;"> {{ $data->qualification }} </span></span></br>
...
@push('scripts')
    <script>
        function printDiv() {
            ...
            var a = window.open('', 'PRINT ADMIT CARD', 'height=330, width=470');
