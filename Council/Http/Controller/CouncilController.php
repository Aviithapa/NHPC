...
class CouncilController extends BaseController
{
    ...
    public function changeDecisionDate()
    {
        if (Auth::user()->mainRole()->name === 'council') {
            ...
            $certificates = $this->certificateRepository->getAll()->where('decision_date', '=', '2023-06-02')
                ->where('program_id', '=', '42');
            $srn =  22594;
    ...
    public function FileChangeCouncilDateApi(Request $request)
    {
        ...
        foreach ($passed_list as $pass) {
            ...
            // Increment $srn by 1 for the next iteration
            $srn++;
        }
    ...
    {
        $data = CertificateHistory::all()->where('decision_date', 'like', '18-01-2023')
            ->orWhere('decision_date', 'like', '2023-01-2023');
