...
class  ExamCommitteeController extends BaseController
{
    ...
    public function FileForwardCouncil()
    {
        ...
        $data['isPassed'] = true;
        $passed_list = ExamResult::all()->where('status', '=', 'PASSED')->where('remarks', '=', '4');
    ...
    public function getAllStudentList()
    {
        ...
        $exams = ExamProcessing::all()->where('state', '=', 'exam_committee')->where('status', '=', 'progress')->where('is_admit_card_generate', '=', 'no')->where('level', '=', '3');
        dd($exams);
